#include <stdio.h>
#include <stdlib.h> // for atof, getline
#include <string.h>
#include <math.h>
...
#define FUNC 'f'    // command for functions
#define VAR 'v'     // command for variables

#define VAR_COUNT 26 // Number of variables (A-Z)

extern double variables[VAR_COUNT];
...
double recent_value = 0.0; // Variable to store the most recent value
...
int main() {
    ...
    // Array to store variables 'A' to 'Z'
        ...
        while ((type = getop(s, &ptr)) != '\0') {
            switch (type) {
                ...
                case FUNC:   // Handle functions
                    ...
                    break;
                case VAR:    // Handle variables
                    if (strlen(s) == 2 && s[0] == '=') {
                        // Set variable (e.g., =A)
                        set_variable(s[1], pop());
                    } else if (strlen(s) == 1 && isupper(s[0])) {
                        // Retrieve variable value (e.g., A)
                    ...
                    break;
                case '\n':
                    recent_value = pop();  // Store the most recent value
